...
static AD5940Err AppRAMPSeqInitGen(void)
    ...
    // lploop_cfg.LpAmpCfg.LpTiaRf = LPTIARF_SHORT;
    ...
    // Calculate initial DAC codes based on RampStartVolt and VzeroStart
    // Aligning logic with RampDacRegUpdate to avoid truncation error mismatches.
    
    // 1. Calculate truncated VzeroCode (State0 logic)
    uint32_t VzeroCode = (uint32_t)((AppRAMPCfg.VzeroStart - 200.0f) / DAC6BITVOLT_1LSB);
    
    // 2. Calculate offset in 12-bit LSBs (RampStartVolt)
    // In RampDacRegUpdate: CurrRampCode = -RampStartVolt / DAC12BITVOLT_1LSB
    int32_t StartVoltCode = (int32_t)(-AppRAMPCfg.RampStartVolt / DAC12BITVOLT_1LSB);
    
    // 3. Calculate VbiasCode relative to truncated VzeroCode
    // In RampDacRegUpdate: VbiasCode = (uint32_t)(VzeroCode * 64 + CurrRampCode)
    uint32_t VbiasCode = (uint32_t)(VzeroCode * 64 + StartVoltCode);

    if (VbiasCode < (VzeroCode * 64)) {
       // Check if this condition from RampDacRegUpdate implies anything specific
       // It seems to just handle the wrap or logic, but let's blindly trust the math for now
       // Actually Vbias < Vzero*64 means (Vzero - Vbias) > 0, which is normal for positive cell voltage
       // But wait, the previous code had: if(VbiasCode < (VzeroCode * 64)) VbiasCode --;
       // That line in RampDacRegUpdate is weird: if(VbiasCode < (VzeroCode * 64)) VbiasCode --;
       // It might be a correction for the 0.2V offset difference or something.
    ...
    lploop_cfg.LpDacCfg.DacData12Bit = VbiasCode; 
    lploop_cfg.LpDacCfg.DacData6Bit = VzeroCode;
...
static AD5940Err RampDacRegUpdate(uint32_t *pDACData)
    ...
    if (VoltageStepCount < SWV_VOLTAGE_STEP_BUFFER_SIZE) {
        VoltageStepBuffer[VoltageStepCount++] = final_cell_voltage_mv;
    }
    //////////////////////////////// END
...
#define CURRBLK_BLK1      1   /* Current block is BLOCK1 */
    ...
    if(AppRAMPCfg.bFirstDACSeq == bTRUE)
        ...
        if (AppRAMPCfg.bRampOneDir)
            ...
            /* NOTE: Inverted signs to match Vcell definition (Vzero - Vbias) */
...
AD5940Err AppRAMPInit(uint32_t *pBuffer, uint32_t BufferSize)
    ...
    AppRAMPCfg.RAMPInited = bFALSE; // Force re-initialization EVERY TIME to flush SINC3 buffer
    if((AppRAMPCfg.RAMPInited == bFALSE) || \
    ...
    /* Manually disable AFE blocks to ensure clean state before SEQ3 */
...
static int32_t AppRAMPDataProcess(int32_t *const pData, uint32_t *pDataCount)
    ...
    for(i = 0; i < datacount; i++)
        {
        pData[i] &= 0xffff;
        pOut[i] = AD5940_ADCCode2Volt(pData[i], AppRAMPCfg.AdcPgaGain, AppRAMPCfg.ADCRefVolt) / AppRAMPCfg.RtiaValue.Magnitude * 1e3f; /* Result unit is uA. */
        }
    return 0;
    }
