...
#define ALIGIN_VOLT2LSB                                                        \
  0 /* Set it to 1 to align each voltage step to 1LSB of DAC. 0: step code is  \
       fractional. */
...
typedef struct {
  ...
  uint32_t SeqStartAddr;    /**< Initialaztion sequence start address in SRAM of
                               AD5940  */
  uint32_t MaxSeqLen;       /**< Limit the maximum sequence.   */
  uint32_t SeqStartAddrCal; /**< Not used for Ramp.Calibration sequence start
                               address in SRAM of AD5940 */
  uint32_t MaxSeqLenCal;    /**< Not used for Ramp. */
                            /* Application related parameters */
  float LFOSCClkFreq; /**< The clock frequency of Wakeup Timer in Hz. Typically
  ...
  float VzeroStart; /**< The start voltage of Vzero in mV. Set it to 2400mV by
                       default */
  float VzeroPeak;  /**< The peak voltage of Vzero in mV. Set it to 200mV by
                       default */
  ...
  float
      ExternalRtiaValue; /**< The optional external RTIA value in Ohm.
                            Disconnect internal RTIA to use external RTIA. When
                            using internal RTIA, this value is ignored. */
  ...
  BoolFlag SWVInited;  /**< If the program run firstly, generated initialization
                          sequence commands */
...
#define APPCTRL_SHUTDOWN                                                       \
  3 /**< Note: shutdown here means turn off everything and put AFE to          \
       hibernate mode. The word 'SHUT DOWN' is only used here. */
